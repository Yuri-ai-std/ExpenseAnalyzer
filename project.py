...
def check_budget_limits(conn, budget_limits, messages, start_date=None, end_date=None):
    """
    Сверяет суммы расходов по месяцам/категориям с лимитами из словаря budget_limits.
    Печатает только превышения. Если для месяца нет лимитов — выводит соответствующее сообщение.
    """
    cur = conn.cursor()
    ...
    if start_date and end_date:
        # строки вида 'YYYY-MM-DD' корректно сравниваются в SQLite
    ...
    printed_no_limits = set()

    for month, category, total in cur.fetchall():
        month_limits = budget_limits.get(month)
        if month_limits is None:
            # Чтобы не спамить, печатаем "no_limits_defined" один раз на месяц
            if month not in printed_no_limits and "no_limits_defined" in messages:
                print(messages["no_limits_defined"].format(month=month))
                printed_no_limits.add(month)
            continue

        limit_val = month_limits.get(category)
        if limit_val is not None and total > limit_val:
            # Сообщение ожидается формата: "{category} ❌ ${total:.2f} > ${limit:.2f}"
            print(
                messages["over_limit"].format(
                    category=category, total=total, limit=limit_val
                )
            )


def update_budget_limits(conn, categories, messages):
    """
    Обновляет лимиты в таблице `limits` (SQLite).
    - спрашивает месяц (YYYY-MM)
    - показывает текущие лимиты
    - для каждой категории предлагает ввести новый лимит (Enter — пропуск)
    - сохраняет в БД (UPSERT по (month, category))
    """
    cursor = conn.cursor()

    # 1) Запрашиваем месяц
    month_str = input(messages["enter_month"]).strip()  # пример: 2025-07

    # 2) Показать текущие лимиты
    print(messages["current_limits"].format(month=month_str))
    cursor.execute(
        "SELECT category, limit FROM limits WHERE month = ? ORDER BY category",
        (month_str,),
    )
    existing = {cat: lim for (cat, lim) in cursor.fetchall()}

    for cat in categories:
        current = existing.get(cat, "not set")
        print(f"  {cat}: {current}")

    # 3) Обновления от пользователя
    for cat in categories:
        while True:
            try:
                raw = input(
                ...
                if raw == "":
                    break  # пропустить категорию
                lim = float(raw)

                # UPSERT по (month, category)
                cursor.execute(
                    """
                    INSERT INTO limits(month, category, limit)
                    VALUES (?, ?, ?)
    ...
    while True:
        ...
        if choice == "1":
            ...
            if filter_choice in ("yes", "oui", "sí", "si"):
                ...
                check_budget_limits(conn, budget_limits, messages, start_date, end_date)
            else:
